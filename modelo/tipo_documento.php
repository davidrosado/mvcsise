...
class tipoDocumento{
    ...
    public function mostrarv2($tabla,$condicion){
        $db = new Database();
        $db->connect();
        $q= "SELECT tipo_documento.IdTipoDocumento, tipo_documento.Descripcion, estado.Descripcion as Estado, tipo_documento.FechaCreacion 
        FROM $tabla  INNER JOIN estado ON estado.IdEstado = tipo_documento.IdEstado
        where ".$condicion." ORDER BY tipo_documento.Descripcion ASC";
