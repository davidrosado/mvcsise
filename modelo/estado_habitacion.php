...
class estadoHabitacion{
    ...
    public function mostrarv2($tabla,$condicion){
        $db = new Database();
        $db->connect();
        $q= "SELECT estado_habitacion.IdEstadoHabitacion, estado_habitacion.Descripcion, estado.Descripcion as Estado, estado_habitacion.FechaCreacion 
        FROM $tabla  INNER JOIN estado ON estado.IdEstado = estado_habitacion.IdEstado
        where ".$condicion." ORDER BY estado_habitacion.Descripcion ASC";
