...
class tipoPersona{
    ...
    public function mostrarv2($tabla,$condicion){
        $db = new Database();
        $db->connect();
        $q= "SELECT tipo_persona.IdTipoPersona, tipo_persona.Descripcion, estado.Descripcion as Estado, tipo_persona.FechaCreacion 
        FROM $tabla  INNER JOIN estado ON estado.IdEstado = tipo_persona.IdEstado
        where ".$condicion." ORDER BY tipo_persona.Descripcion ASC";
