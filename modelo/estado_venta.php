...
class estadoVenta{
    ...
    public function mostrarv2($tabla,$condicion){
        $db = new Database();
        $db->connect();
        $q= "SELECT estado_venta.IdEstadoVenta, estado_venta.Descripcion, estado.Descripcion as Estado, estado_venta.FechaCreacion 
        FROM $tabla  INNER JOIN estado ON estado.IdEstado = estado_venta.IdEstado
        where ".$condicion." ORDER BY estado_venta.Descripcion ASC";
