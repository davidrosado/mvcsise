...
class estadoRecepcion{
    ...
    public function mostrarv2($tabla,$condicion){
        $db = new Database();
        $db->connect();
        $q= "SELECT estado_recepcion.IdEstadoRecepcion, estado_recepcion.Descripcion, estado.Descripcion as Estado, estado_recepcion.FechaCreacion 
        FROM $tabla  INNER JOIN estado ON estado.IdEstado = estado_recepcion.IdEstado
        where ".$condicion." ORDER BY estado_recepcion.Descripcion ASC";
